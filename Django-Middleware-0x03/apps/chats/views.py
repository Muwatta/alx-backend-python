...
class ConversationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return self.queryset.filter(participants=user).prefetch_related('participants', 'messages__sender').order_by('-created_at')
    ...
    def admin_action(self, request):
        """Admin action endpoint for testing role-based permissions."""
        return Response({"message": "Admin action performed"}, status=status.HTTP_200_OK)
